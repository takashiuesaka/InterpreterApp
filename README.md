...
現在は、Microsoft Learn の GPT Realtime Translate ガイドに合わせて、
マイク音声を Realtime API へストリーミングし、日本語翻訳テキストを逐次表示する実装です。

## MVP 検証観点

本プロジェクトの MVP は、UI の見た目ではなく以下の技術検証を満たすことを重視します。

1. Microsoft Foundry 上の「リアルタイム翻訳機能を持つ GPT モデル」と実際に接続できること
...
補足:
- `FOUNDRY_DEPLOYMENT` には `gpt-realtime-translate` のデプロイ名を指定
- 接続先は `wss://<resource>.openai.azure.com/openai/v1/realtime/translations?model=<deployment>` 形式 (GA)
